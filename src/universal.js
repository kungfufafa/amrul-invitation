...
var init_selectize = function (el, opt) {
  try {
    if (el && typeof $.fn.selectize === "function") {
      ...
      console.log(
        "⚠️ Selectize.js not available, falling back to regular select"
      );
      return null;
    }
  } catch (error) {
      ...
      render: {
        item: function (item, escape) {
          return (
            "<div>" +
            "<p>" +
            (item.title ? escape(item.title) : "(Pilih Acara Utama)") +
            "</p>" +
            "</div>"
          );
        },
...
var main_event_selection = function (
  el,
  data = [],
  sel = [],
  callback = () => {}
  ...
  try {
    ...
    if (el && options != "") {
      ...
      if (selectize) {
        ...
        console.log(
          "⚠️ Selectize failed, using fallback for main_event select"
        );
...
function universalSearch(list, keyword, fields) {
  const qTokens = keyword.toLowerCase().trim().split(/\s+/);
  ...
  return list.filter((item) => {
    ...
    return qTokens.every((q) => hTokens.some((h) => h.includes(q)));
  });
}

// Extract Main Domain
window.extractMainDomain = function (url) {
...
$(document).ready(function () {
  ...
  $(window).on("click", function (e) {
    // Hide Dropdown
    if (
      !$(e.target).closest(".dropdown-btn, .dropdown-body.filter-col").length
    ) {
      hide_dropdown();
    }
  });
